  ...
  try {
    await new Promise<void>((resolve, reject) => {
      try {
        ...
        doc.on("end", () => {
          console.log(`[Ticket API] PDF generation finished for ${ticket.ticketNumber}`);
          resolve();
        });
        doc.on("error", (err) => {
          console.error(`[Ticket API] PDF Stream Error:`, err);
          reject(err);
        });
    ...
    console.error(`[Ticket API] Critical Failure:`, err);
    return apiError("Failed to generate ticket PDF", 500);
  }
