...
export async function GET(
  ...
  console.log(`[Statement API] Request received for ID: ${id}`);
  
  const session = await auth();
  if (!session?.user) {
    console.warn(`[Statement API] Unauthorized request for ID: ${id}`);
  ...
  console.log(`[Statement API] Generating PDF for order ${order.orderNumber}`);
  const chunks: Buffer[] = [];
  ...
  try {
    await new Promise<void>((resolve, reject) => {
      try {
        ...
        doc.on("end", () => {
          console.log(`[Statement API] PDF generation finished for ${order.orderNumber}`);
          resolve();
        });
        doc.on("error", (err) => {
          console.error(`[Statement API] PDF Stream Error:`, err);
          reject(err);
        });
    ...
    console.error(`[Statement API] Critical Failure:`, err);
    return apiError("Failed to generate PDF statement", 500);
  }
